...
def test_no_artificial_zeros_inside_physical_disk(tmp_path):
    ...
    r = np.sqrt(X * X + Y * Y)
    interior = r <= 0.95
    # Every interior cell should have SOME non-zero value at some
    # timestep. Zeroed interior would indicate an unintended mask.
    per_cell_max_abs = np.max(np.abs(f), axis=-1)
    zero_interior = (per_cell_max_abs == 0) & interior
    assert not zero_interior.any(), (
        f"{zero_interior.sum()} interior cells (r<=0.95) are all-zero "
        f"across time. Loader is masking data it shouldn't.")
